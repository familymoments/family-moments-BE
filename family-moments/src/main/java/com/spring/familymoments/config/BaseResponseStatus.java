...
public enum BaseResponseStatus {
    ...
    POST_USERS_FAILED_TO_VERIFY(false, HttpStatus.BAD_REQUEST.value(), "'인증받기' 버튼을 눌러 이메일 인증을 완료해주세요."),
    POST_USERS_FAILED_TO_COMPLETE_VERIFY(false, HttpStatus.BAD_REQUEST.value(), "이메일로 전송된 인증 번호를 입력한 후, '인증확인' 버튼을 눌러주세요."),
    EMPTY_VERIFICATION_CODE(false, HttpStatus.BAD_REQUEST.value(), "받은 인증 번호를 입력해주세요."),
    INVALID_VERIFICATION_CODE(false, HttpStatus.BAD_REQUEST.value(), "인증 번호는 6자리입니다. 다시 입력해주세요."),
    ...
    minnie_POSTS_FULL_IMAGE(false, HttpStatus.BAD_REQUEST.value(), "한 게시물에 포함할 수 있는 이미지의 개수는 최대 4장입니다."),
